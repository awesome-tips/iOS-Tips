...
Swift 4中的Dictionary字典新增了一个mapValue(_:)方法，来转换字典的值，同时保留key。如代码清单13-1-1所示。
...
使用`Unicode`码显得不直观，所以我们可以自定义图标库。`react-native-vector-icons`有一些内置的图标库，我们参考其创建方式，我们以Zocial为例，如代码清单27-1-1所示：
